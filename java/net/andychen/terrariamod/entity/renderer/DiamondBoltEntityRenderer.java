...
public class DiamondBoltEntityRenderer extends EntityRenderer<DiamondBoltEntity> {
    ...
    private final StaffProjectileEntityModel<DiamondBoltEntity> model;

    public DiamondBoltEntityRenderer(EntityRendererFactory.Context context) {
        super(context);
        this.model = new StaffProjectileEntityModel(context.getPart(TerrariaClientMod.MODEL_DIAMOND_BOLT_LAYER));
    }

    public void render(DiamondBoltEntity staffProjectileEntity, float f, float g, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i) {
        matrixStack.push();
    ...
    public Identifier getTexture(DiamondBoltEntity staffProjectileEntity) {
        return TEXTURE;
    }
}
