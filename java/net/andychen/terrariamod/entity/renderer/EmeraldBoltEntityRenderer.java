...
public class EmeraldBoltEntityRenderer extends EntityRenderer<EmeraldBoltEntity> {
    ...
    private final StaffProjectileEntityModel<EmeraldBoltEntity> model;

    public EmeraldBoltEntityRenderer(EntityRendererFactory.Context context) {
        super(context);
        this.model = new StaffProjectileEntityModel(context.getPart(TerrariaClientMod.MODEL_EMERALD_BOLT_LAYER));
    }

    public void render(EmeraldBoltEntity staffProjectileEntity, float f, float g, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i) {
        matrixStack.push();
    ...
    public Identifier getTexture(EmeraldBoltEntity staffProjectileEntity) {
        return TEXTURE;
    }
}
