...
import net.andychen.terrariamod.item.ModArmorMaterial;
...
public class CrimsonArmorItem extends ArmorItem {
    private static final Map<ArmorMaterial, StatusEffect> MATERIAL_TO_EFFECT_MAP =
            (new ImmutableMap.Builder<ArmorMaterial, StatusEffect>())
                    .put(ModArmorMaterial.CRIMSON, StatusEffects.REGENERATION).build();

    public CrimsonArmorItem(ArmorMaterial material, EquipmentSlot slot, Settings settings) {
    ...
    private void addStatusEffectForMaterial(PlayerEntity player, ArmorMaterial mapArmorMaterial,
                                            StatusEffect mapStatusEffect) {
        boolean hasPlayerEffect = player.hasStatusEffect(mapStatusEffect);

        if(hasCorrectArmorOn(mapArmorMaterial, player) && !hasPlayerEffect) {
            player.addStatusEffect(new StatusEffectInstance(mapStatusEffect, 40, 0, false, false));
